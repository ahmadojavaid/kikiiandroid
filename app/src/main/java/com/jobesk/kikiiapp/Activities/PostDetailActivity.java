...
public class PostDetailActivity extends AppCompatActivity implements View.OnClickListener, CommentsAdapter.IClicks {
    ...
    private void setPostData() {
        tv_name.setText(singlePost.getUser().getName());

        if (singlePost.getBody() != null) {
            tv_description.setText(singlePost.getBody().replace("\"", ""));
        }


        tv_time_ago.setText(singlePost.getCreatedAt());
        Glide.with(context)
                .load(singlePost.getUser().getProfilePic())
                .centerCrop()
                .dontAnimate()
        ...
        if (singlePost.getMedia().size() > 0) {
            img_media.setVisibility(View.VISIBLE);
            Glide
                    .with(context)
                    .load(singlePost.getMedia().get(0).getPath())
                    .centerCrop()
                    .dontAnimate()
                    .diskCacheStrategy(DiskCacheStrategy.RESOURCE)
                    .centerCrop()
...
//        });
    ...
    public void onMenuClick(View view, PostComment data, int position) {
        showCommentMenu(activity, view, data, position, rv_comments, commentsList, commentsAdapter, tv_no);
    }
        ...
        switch (v.getId()) {
            ...
            case R.id.tv_update_comment:
                if (et_comment.getText().toString().isEmpty()) {
                    ...
                    updateCommentParams.put(Constants.POST_ID, postID);
                    updateComment();
                }
                break;
            case R.id.img_post_menu:
                ShowPopupMenus.showPostMenu(activity, img_post_menu, singlePost, et_comment, comment);
                break;
            case R.id.img_back:
                onBackPressed();
