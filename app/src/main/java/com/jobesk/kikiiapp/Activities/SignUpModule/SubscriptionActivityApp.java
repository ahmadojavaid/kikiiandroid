...
public class SubscriptionActivityApp extends AppCompatActivity implements ViewPager.OnPageChangeListener, View.OnClickListener, PurchasesUpdatedListener {

    private static final String TAG = "SubscriptionActivityApp";
    private Context mContext = SubscriptionActivityApp.this;
    ...
    private void addBilling() {
        ...
        billingClient = BillingClient.newBuilder(SubscriptionActivityApp.this).enablePendingPurchases().setListener(new PurchasesUpdatedListener() {
            @Override
            ...
            public void onPurchasesUpdated(BillingResult billingResult, @Nullable List<Purchase> list) {
                if (list != null && billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                    ...
                    if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.USER_CANCELED) {
                        Toast.makeText(SubscriptionActivityApp.this, "Try Purchasing Again", Toast.LENGTH_SHORT).show();
                    } else {
                        if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.ITEM_ALREADY_OWNED) {
                            Toast.makeText(SubscriptionActivityApp.this, "Already Purchased", Toast.LENGTH_SHORT).show();
...
//                    Toast.makeText(SubscriptionActivityApp.this, "Successfully connected to Billing client", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(SubscriptionActivityApp.this, "Failed to connect", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onBillingServiceDisconnected() {
                Toast.makeText(SubscriptionActivityApp.this, "Disconnected from the Client", Toast.LENGTH_SHORT).show();
            }
        });
        skulist.add(product);
...
//                            Toast.makeText(SubscriptionActivityApp.this, "Purchase Acknowledged", Toast.LENGTH_SHORT).show();
                        }
                    };
    ...
    private void updatePurchaseOnserver() {
        ...
        API api = RestAdapter.createAPI(SubscriptionActivityApp.this);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.no_thanks_tv:
                ...
                if (value.equalsIgnoreCase("meetfrag")) {
                ...
                if (value.equalsIgnoreCase("editProfile")) {
                    finish();
                    return;
                }
                if (value.equalsIgnoreCase("support")) {
                    finish();
                    return;
                }
                        ...
                        if (list != null && billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                            for (final SkuDetails skuDetails : list) {
                                ...
                                BillingFlowParams flowParams = BillingFlowParams.newBuilder()
                                        ...
                                        .build();
                                BillingResult responsecode = billingClient.launchBillingFlow(SubscriptionActivityApp.this, flowParams);
                            }
                        }
                    }
                });
